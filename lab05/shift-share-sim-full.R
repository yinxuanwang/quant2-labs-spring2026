...
# Extract the other country's shift
# Rows are industries, columns are time periods
shift_OC_mat <- df_shifts |> 
  select(time, ind, shift_OC) |> 
  pivot_wider(names_from = time, values_from = shift_OC) |> 
  select(-ind) |> 
  as.matrix()

# Multiply the base shares (N x J) matrix by the OC shifts matrix (J x T) 
# Result is an N x T matrix of the instrument Z
Z_mat <- shares %*% shift_OC_mat

# Add to panel
df_panel$Z <- as.numeric(Z_mat)
...
# OLS
ols <- feols(Y ~ D + X1 + X2 + X3 | region + time, cluster = ~region, data = df_panel)

# 2SLS
iv  <- feols(Y ~ X1 + X2 + X3 | region + time | D ~ Z, cluster = ~region, data = df_panel)
